...
run_binary() {
    ...
    case "$(basename "$binary")" in
        ...
        *) 
            ...
            status="ER"
            echo "Running command: ASAN_OPTIONS=detect_leaks=0 $cmdline $status" | tee -a "$output_file"
            return 1
            ;;
    esac
...
{
    if ($0 ~ /^Running command:/) {
        # 바이너리 이름 추출
        match($0, /bin\/[^\/]+\/([^ <]+)/, arr1)
        binary = (arr1[1] != "") ? arr1[1] : ""
        ...
        # 상태 추출
        match($0, /(SU|ER|TO)$/, arr3)
        status = (arr3[1] != "") ? arr3[1] : ""
...
echo "All executions and summary completed. Results are stored in $results_dir directory."
