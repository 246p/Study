...
run_binary() {
    ...
    echo -n "Running command: ASAN_OPTIONS=detect_leaks=0 $cmdline" | tee -a "$output_file"
    ...
    for binary in bin/$dir/*; do
        ...
        for poc in $poc_files; do
            ...
            if [ $exit_code -eq 124 ]; then
                echo " TO" | tee -a "$output_file"
            elif [ $exit_code -ne 0 ]; then
                echo " ER" | tee -a "$output_file"
            else
                echo " SU" | tee -a "$output_file"
            fi
        done
    done
done
