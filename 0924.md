...
# 바이너리 디렉토리 및 PoC 디렉토리
# binary_dirs=("AFL" "AFLGo" "WindRanger" "vanilla")
...
run_binary() {
    ...
    case "$(basename "$binary")" in
        ...
        *) 
            echo "Unknown binary type: $(basename "$binary")"
            return 1
            ;;
    esac
    echo -n "ASAN_OPTIONS=detect_leaks=0 $cmdline"
    eval "timeout $timeout_duration bash -c '$cmdline'" > "$output_file" 2>&1
    exit_code=$?
...
echo "All executions completed. Results are stored in $results_dir directory."
