# TODO
- ASAN Option 설정 (traige, fuzzing) : `ASAN_OPTIONS=detect_leaks=0,allocator_may_return_null=1`
- commit 정리
- malloc hooking
- triage에서 ASAN log가 있는지 확인하는 방향으로 변경
- windranger build시 benchmark-project/*/build.sh에서 -fsanitize=address 꺼야 build가 됨 (수정 필요)
# CVE
## test
- sh
``` sh
...
### 2016-9831 - BOF
### 2017-9988 - ND
- `ASAN_OPTIONS=detect_leaks=0,allocator_may_return_null=1 bin/vanilla/swftophp-20179988 poc/libming-4
.7/20179988-repro`
- poc-github
``` log
root@babb029b36d6:/benchmark# ASAN_OPTIONS=detect_leaks=0,allocator_may_return_null=1 bin/vanilla/swftophp-20179988 poc/libming-4.7/20179988-github
...
nsset->Count : 0xa0c99315
nsset->NS : 0x7ef9ab835800
```
- poc-repro
``` log
root@babb029b36d6:/benchmark# ASAN_OPTIONS=detect_leaks=0,allocator_may_return_null=1 bin/vanilla/swftophp-20179988 poc/libming-4.7/20179988-repro
...
nsset->Count : 0x1a
nsset->NS : 0x611000000680
nsset->Count : 0xffffffff
nsset->NS : 0x7f26931ff800
```
- `ulimit -v 102400` 로 virtual memory 제한 하고자 하였지만 ASAN error (Shadow memory mapping)
``` log
==33265==ERROR: AddressSanitizer failed to allocate 0x10000000 (268435456) bytes at address 7fff7000 (errno: 12)
==33265==ReserveShadowMemoryRange failed while trying to map 0x10000000 bytes. Perhaps you're using ulimit -v
Aborted
```
- docker 실행시 memory 제한 : `docker run --name test --user root -m=1g --rm -it dfuzz-exp`
```
nsset->Count : 0xa0c99315
Killed
...
...
Killed
```
- Killed `$?=137` = OOM (Out of Memory) : gdb log
``` log
2994      nsset->NS = malloc(sizeof(U30) * nsset->Count);
(gdb) n
...
``` log
==13==ERROR: AddressSanitizer failed to allocate 0xdfff0001000 (15392894357504) bytes at address 2008fff7000 (errno: 12)
==13==ReserveShadowMemoryRange failed while trying to map 0xdfff0001000 bytes. Perhaps you're using ulimit -v
```
- ASAN에서 1.5기가 정도의 shadow memory를 할당하려고함
- overcommit_memory = 2 : overcommit을 하지 않아서 swap space + physical memory /2를 할당
- ISLAB setting
``` log
               total        used        free      shared  buff/cache   available
Mem:           503Gi        15Gi       380Gi       1.4Gi       106Gi       481Gi
...
## llvm pass
- malloc, free hooking 하여 memmory tracking(address, size)
