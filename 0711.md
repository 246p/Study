...
- fixed point가 되지 않으면 어떻게 처리하는가? 
- 10번 반복 후에 fixed point에 도달하지 못하면 분석을 종료하고 symbolic condition C를 생성하지 않음
> our current implemented algorithm terminates the analysis and fails to generate a symbolic condition C if it fails to reach a fixed point after ten iterations.
- 특정 변수에 대한 조건이 변화하는 것이기 때문에 한 loop에서 특정 변수가 10번 이상 condition 변화가 생기는 일이 잘 없다
...
- Inp(f) : input field f를 f의 모든 instantitaion value에 mapping
...
2. load, store 값을 pointer or alias analysis로 얻는 정보로 equivalence class로 group화 함 > 보수적 모델링
...
-  `(σ, ς, h, Inp) |= C` : abstract program state가 condition C를 만족한다.
