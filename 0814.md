...
def worker(worker_id, fuzzer, timelimit, outdir):
    while True:
        try:
            ...
            cmdline = "'" + cmdline + "'" # single quote 추가
...
```
## fuzzer script
- ""를 통해 cmdline format을 string으로 넘겨줌
- $3, $4는 WindRanger에서 copy distance 를 위한 argument
``` sh
# run_fuzzer.sh
/fuzzer/script/run_$1.sh $2 "$5" $3 $4
```
- `echo $2`를 통해 string을 다시 argument로 나누어줌
``` sh
# run_AFL.sh
timeout $1 /fuzzer/AFL/afl-fuzz -m none -d -i seed -o /output -- ./target `echo $2`
# run_WindRanger.sh
