...
# find extract
``` ocaml
let find_precond_backward (c: Constraint.t) (node: InterCfg.Node) (state: AbsDom.state) : Constraint.t =
    let cmd = InterCfg.cmdof global.icfg node in
    match cmd with
    (* 
        Cset = lval * exp * location
        lv = lhost * offset 
        lhost = Var of varinfo | Mem ofr exp
        offset = NoOffset | Field of fieldinfo * offset | Index of exp * offset
    *)
    | IntraCfg.Cmd.Cset ((Var v, NoOffset), e, _) ->  (* v = e *)
        replace_constraint c (Var v) e
    | IntraCfg.Cmd.Cset ((Mem e1, NoOffset), e2, _) -> (* *e1 = e2 *) 
        let x = e1's candidate
        replace_constraint c (Deref e1) e2
        c (* pointer analysis - preanalysis, itvSem.ml code 사용 *)
    | IntraCfg.Cmd.Cset ((Var v, offset), e, _) -> (* v.offset = e *)
        c (* constraint에서 v.offset을 e로 변경, replace_expr update 필요 *)
    | IntraCfg.Cmd.Cset ((Var v, Index (exp, NoOffset)), e, _) (* v[exp] = e*)
        c (* constraint에서 v[exp]를 e로 변경, replace_expr update 필요 *)
    (* Cset의 다른 case도 존재함 *)
    | _ -> c
```
# pointer aliase
...
- 이러한 설계 선택에 대한 side effect는 잘 모르겠습니다. (RHS의 pointer에 의해 발생)
