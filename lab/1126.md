...
# find extract
``` ocaml
let find_precond_backward (c: Constraint.t) (node: InterCfg.Node) (state: ItvDom.Mem) : Constraint.t =
    ...
    | IntraCfg.Cmd.Cset ((Mem e1, NoOffset), e2, _) -> (* *e1 = e2 *) 
        ...
        List.fold_left (fun acc x -> replace_constraint acc x e2) c alias in
    | IntraCfg.Cmd.Cset ((Var v, offset), e, _) -> (* v.offset = e *)
        replace_constraint c (Var v) e
    | IntraCfg.Cmd.Cset ((Mem e1, offset), e2, _) -> (* e1->offset = e2 *)
        (* ->의 경우에도 pointer와 유사하게 alias 적용 필요*)
